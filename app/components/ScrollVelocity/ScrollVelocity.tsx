...
/* Adapted from React Bits (reactbits.dev) "ScrollVelocity". */
...
function useElementWidth(ref: RefObject<HTMLSpanElement | null>) {
  ...
  useLayoutEffect(() => {
    function updateWidth() {
      if (ref.current) {
        // getBoundingClientRect for subpixel precision — offsetWidth rounds to
        // integers, which makes the wrap-around jump visibly on every loop.
        setWidth(ref.current.getBoundingClientRect().width);
      }
    }
    updateWidth();
    // Re-measure once webfonts finish loading, otherwise the fallback font's
    // width sticks and the loop point is wrong.
